...
class _MyHomePageState extends State<MyHomePage> {
  Future<Weather?> fetchData() async {
    Dio dio = Dio();
    // // dio- бул беккенд тараптагы серверди тартып келуучу плагин
          ...
          child: FutureBuilder(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.none) {
                //ConnectionState.none - интернет менен байланыш жок
                return const Text('Сизде интернет байланышы узулду');
              } else if (snapshot.connectionState == ConnectionState.waiting
                  //ConnectionState.waiting- загрузка болгондо
                  ) {
                return const CircularProgressIndicator();
              } else if (snapshot.connectionState == ConnectionState.done) {
                // ConnectionState.done- запрос аткарылды
